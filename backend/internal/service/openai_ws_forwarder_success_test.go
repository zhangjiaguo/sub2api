...
func TestOpenAIGatewayService_BuildOpenAIWSHeadersDeviceModePreservesNamespacedClientSessionIdentity(t *testing.T) {
	...
	require.NoError(t, err)
	require.Empty(t, headers.Get("x-codex-installation-id"), "installation-id 不再直发头（codex-rs 0.15x 只在 body client_metadata）")
	...
	// 会话三件套由方言层以隔离值重建（与 HTTP 转发同一公式），device 模式不覆盖。
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "client-session"), headers.Get("session-id"))
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "client-thread"), headers.Get("thread-id"))
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "client-thread"), headers.Get("x-client-request-id"))
}
...
func TestOpenAIGatewayService_Forward_WSv2_OAuthStoreFalseByDefault(t *testing.T) {
	...
	// OAuth 账号的会话头按 API key 和上游账号隔离（codex-rs 0.15x 连字符方言），
	// 测试中未设置 api_key 到 context，apiKeyID=0。conversation 头已废弃，
	// 对话标识只在 body 的 prompt_cache_key。
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "sess-oauth-1"), captureDialer.lastHeaders.Get("session-id"))
...
func TestOpenAIGatewayService_Forward_WSv2_OAuthOriginatorCompatibility(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, codexAccountOverrideUserAgent(account), captureDialer.lastHeaders.Get("user-agent"))
...
func TestOpenAIGatewayService_Forward_WSv2_HeaderSessionFallbackFromPromptCacheKey(t *testing.T) {
	...
	// OAuth 账号的 session-id 应同时按 API key 和上游账号隔离（apiKeyID=0）。
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "pcache_123"), captureDialer.lastHeaders.Get("session-id"))
	require.Empty(t, captureDialer.lastHeaders.Get("session_id"), "下划线会话头已废弃")
	require.Empty(t, captureDialer.lastHeaders.Get("conversation_id"))
	require.NotNil(t, captureConn.lastWrite)
...
func TestOpenAIGatewayService_Forward_WSv2_CodexFingerprintHandshakeBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	require.Empty(t, captureDialer.lastHeaders.Get("x-codex-installation-id"), "installation-id 只在 body client_metadata，不发独立头")
