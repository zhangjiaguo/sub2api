...
func TestOpenAIAgentIdentityPassthroughKeepsSessionAndPromptCacheHeaders(t *testing.T) {
	...
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "client-session"), req.Header.Get("session-id"))
	...
	require.Empty(t, oauthReq.Header.Get("session_id"))
	require.Empty(t, oauthReq.Header.Get("conversation_id"))
}
...
func TestOpenAIAgentIdentityChatRecoveryKeepsAutoDerivedSessionIsolationStable(t *testing.T) {
	...
	require.Equal(t, isolateOpenAIUpstreamSessionID(99, codexAccountIdentitySource(c, account), firstKey), upstream.requests[0].Header.Get("session-id"))
